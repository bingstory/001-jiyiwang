...
class EnlightenController extends CommonController{
	//新闻列表
	public function index(){
       import('Class.Category');
       $cate = M('category')->select();
       $this->category = \Category::getChild($cate,28); 

       $cate_id = I('cate_id','','intval'); 
       $this->assign('cate_id',$cate_id);
       $where2 = empty($cate_id) ? '' : array('cid'=>$cate_id);
       $this->c_page('enlighten','EnlightenRelation','list','pubtime desc',$where2); 
	}

	//回收站
	public function trash(){
       $this->news = D('EnlightenRelation')->getNewss(1); 
       $this->display('index');
	}
	...
	public function toTrash(){ 
		$type = (int)$_GET['type'];
		$msg = $type ? "删除" : "还原";
		$update = M('enlighten')->where(array('id'=>(int)$_GET['id']))->setField('del',$type); 
		if($update){ 
    ...
    public function bombTrash(){
    	$result = D('EnlightenRelation')->relation('true')->where(array('del'=>1))->delete();
    	if($result){
    		$this->success('清空成功',U(MODULE_NAME.'/Enlighten/trash'));
    	}else{
    		$this->error('清空失败');
    	}
    }
	...
	public function doAdd(){ 
		...
		if(M('enlighten')->add($data)){
			$this -> success('添加成功',U(MODULE_NAME.'/Enlighten/index'));
		}else{
	        $this->error('添加失败');
		}
    ...
    public function edit(){ 
       ...
       $id = I('id');
       $this->news = D('EnlightenRelation')->relation(true)->find($id);
    ...
    public function doEdit(){
    	...
    	if(M('enlighten')->save($data) !== false){
    		$this->success('修改成功',U(MODULE_NAME.'/Enlighten/index',array('cate_id'=>$cate_id)));
    	}else{
    		$this->error('修改失败');
    	} 
    }
