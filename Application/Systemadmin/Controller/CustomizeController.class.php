...
class CustomizeController extends CommonController{
	//新闻列表
	public function index(){
       import('Class.Category');
       $cate = M('category')->select();
       $this->category = \Category::getChild($cate,2); 

       $cate_id = I('cate_id','','intval'); 
       $this->assign('cate_id',$cate_id);
       $where2 = empty($cate_id) ? '' : array('cid'=>$cate_id);
       $this->c_page('customize','CustomizeRelation','news','pubtime desc',$where2); 
	}

	//回收站
	public function trash(){
       $this->news = D('CustomizeRelation')->getNewss(1); 
       $this->display('index');
	}
	...
	public function toTrash(){ 
		$type = (int)$_GET['type'];
		$msg = $type ? "删除" : "还原";
		$update = M('customize')->where(array('id'=>(int)$_GET['id']))->setField('del',$type); 
		if($update){ 
    ...
    public function bombTrash(){
    	$result = D('CustomizeRelation')->relation('true')->where(array('del'=>1))->delete();
    	if($result){
    ...
    $upload = new \Think\Upload();                             // 实例化上传类    
		...
		if(M('customize')->add($data)){
			$this -> success('添加成功',U(MODULE_NAME.'/Customize/index'));
	}else{
       $this->error('添加失败');
	}
    ...
    public function newsEdit(){ 
       ...
       $id = I('id');
       $this->news = D('CustomizeRelation')->relation(true)->find($id);
    ...
    public function doNewsEdit(){
      ...
      $upload = new \Think\Upload();                             // 实例化上传类    
    	...
    	// p($data);die;
    	if(M('customize')->save($data) !== false){
    		$this->success('修改成功',U(MODULE_NAME.'/Customize/index',array('cate_id'=>$cate_id)));
    	}else{
    		$this->error('修改失败');
    	} 
    }
