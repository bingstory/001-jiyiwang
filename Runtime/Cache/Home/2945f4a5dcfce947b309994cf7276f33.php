...
function MM_swapImage() { //v3.0
  var i,j=0,x,a=MM_swapImage.arguments; document.MM_sr=new Array; for(i=0;i<(a.length-2);i+=3)
   if ((x=MM_findObj(a[i]))!=null){document.MM_sr[j++]=x; if(!x.oSrc) x.oSrc=x.src; x.src=a[i+2];}
        ...
        <script>
           $(function(){
           	var navtag = $('#navtag').attr('value');alert(navtag);
...
<div id="star_box2">
    ...
    <div class="st_b2_title"><span class="fl_l st2_l"><span class="fl_l"><img src="/001-jiyiwang/Public/Home/images/cp1_19.jpg" width="27" height="27" /></span> &nbsp;&nbsp;<span class="fl_r"><strong><?php if(empty($position)): ?>个性化定制<?php else: echo ($position); endif; ?></strong></span></span><span class="fl_r c2_r">位置：<a href="<?php echo U('/Home/c');?>">个性化定制</a><?php if(empty($position)): else: ?> > <?php echo ($position); endif; ?></span></div>
        <div id="gao"></div>
        <div class="gx_pic">
        <ol>
        <?php if(is_array($list)): foreach($list as $key=>$v): ?><li>
<span class="fl_l"><a href="<?php echo U('/Home/cs_'.$v['id']);?>"><?php echo ($key+1); ?>, <?php echo (msubstr($v["title"],0,40,0)); ?></a></span>
<span class="fl_r"><a href="<?php echo U('/Home/cs_'.$v['id']);?>"><?php echo (date("Y-m-d H:i",$v["pubtime"])); ?></a></span>
