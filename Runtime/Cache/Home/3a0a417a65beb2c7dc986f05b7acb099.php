...
<div id="star_box2">
    ...
    <div class="st_b2_title"><span class="fl_l st2_l"><span class="fl_l"><img src="/001-jiyiwang/Public/Home/images/cp1_19.jpg" width="27" height="27" /></span> &nbsp;&nbsp;<span class="fl_r"><strong><?php if(empty($position)): ?>集艺新闻<?php else: echo ($position); endif; ?></strong></span></span><span class="fl_r c2_r">位置：<a href="<?php echo U('/Home/news');?>">集艺新闻</a><?php if(empty($position)): else: ?> > <?php echo ($position); endif; ?></span></div>
        <div id="gao"></div>
        <div class="gx_pic">
        <ol>
        <?php if(is_array($list)): foreach($list as $key=>$v): ?><li>
<span class="fl_l"><a href="<?php echo U('/Home/ns_'.$v['id']);?>"><?php echo ($key+1); ?>, <?php echo (msubstr($v["title"],0,40,0)); ?></a></span>
<span class="fl_r"><a href="<?php echo U('/Home/ns_'.$v['id']);?>"><?php echo (date("Y-m-d H:i",$v["pubtime"])); ?></a></span>
 ...
 <?php echo ($page); ?>
  ...
  <div class="vtitle">集艺新闻</div>
    <div class="vcon">
    <ul class="vconlist clearfix">
    <?php if(is_array($nav)): foreach($nav as $key=>$v): ?><li <?php if($key == 0): ?>class="one"<?php else: ?>class="three"<?php endif; ?>><a href="<?php echo U('/'.MODULE_NAME.'/n_'.$v['id']);?>"><?php echo ($v["name"]); ?></a></li><?php endforeach; endif; ?>
      <!-- <li class="three"><a href="javascript:;">我要定制</a></li> -->
    </ul>
  </div>
